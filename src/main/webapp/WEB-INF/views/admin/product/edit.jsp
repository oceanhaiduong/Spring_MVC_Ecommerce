								...
								<div class="card-body">
									<form 
										...
										<div class="row gx-3 mb-3">
											...
											<div class="col-md-6">
												<label class="small mb-1" for="inputUsername">Tiêu
													đề</label> <input name="title" value="${productDetail.title}"
											...
											<div class="col-md-2">											
												<input hidden="hidden" class="form-control" name="id" value="${id}" type="text" /> 
											</div>
										</div>
										<!-- Form Group (username)-->
										<div class="mb-3">
													...
													<c:when test="${categories != null}">
														<select name="category_id" style="margin-top: 5px;"
															...
															<c:forEach var="category" items="${categories}">
																<option value="${category.id_type}" ${category.id_type == productDetail.category_id ? 'selected' : ''}>${category.name_type}</option>
															</c:forEach>
														</select>
													</c:when>
													<c:otherwise>
